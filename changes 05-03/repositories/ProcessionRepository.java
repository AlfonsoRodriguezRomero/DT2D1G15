...
public interface ProcessionRepository extends JpaRepository<Procession, Integer> {
	...
	Collection<Procession> findAllInFinalMode();
	...
	//select p from Procession p where p.moment between current_timestamp() and dateadd(day,30,current_timestamp())
	//select p from Procession p where p.moment > current_timestamp()
}
