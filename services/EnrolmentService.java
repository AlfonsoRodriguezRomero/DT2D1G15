...
public class EnrolmentService {
	...
	public Enrolment save(final Enrolment enrolment) {
		...
		//his or her brotherhoods list must remove the enrolment brotherhood
		if (enrolment.getId() != 0)
			if (!enrolment.getDropOutMoment().equals(null)) {
				final Member m = enrolment.getMember();
				final Collection<Brotherhood> bs = new ArrayList<>();
				final Collection<Brotherhood> memberBrotherhoods = m.getBrotherhoods();
				bs.addAll(memberBrotherhoods);
				final Brotherhood b = enrolment.getBrotherhood();
				bs.remove(b);
				m.setBrotherhoods(bs);
			}
